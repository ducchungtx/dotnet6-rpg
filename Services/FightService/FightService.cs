...
namespace first_api.Services.FightService
{
  public class FightService : IFightService
  {
    ...
    public async Task<ServiceReponse<FightResultDto>> Fight(FightRequestDto request)
    {
      var response = new ServiceReponse<FightResultDto>
      {
      ...
      try
      {
        ...
        while (!defeated)
        {
          foreach (var attacker in characters)
          {
            ...
            response.Data.Log.Add($"{attacker.Name} attacks {opponent.Name} using {attactUsed} with {(damage > 0 ? damage : 0)} damage");
    ...
    public async Task<ServiceReponse<List<HighScoreDto>>> GetHighScore()
    {
      var characters = await _context.Characters
        .Where(c => c.Fights > 0)
      ...
      var response = new ServiceReponse<List<HighScoreDto>>(){
